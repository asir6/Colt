...
BGD_DECLARE(void) gdImageLine (gdImagePtr im, int x1, int y1, int x2, int y2, int color)
{
    ...
    else
    {
        ...
        if (y1 < -offset && y2 < -offset)
            return;
        if (x1 > im->cx2+offset && x2 > im->cx2+offset)
            return;
        if (y1 > im->cy2+offset && y2 > im->cy2+offset)
            return;
    }
    ...
    {
        /* special case of a thin horizontal line with simple color */
        /* -- this is worth optimizing and inlining */
        /* since polygon scanlines are drawn using horizontal lines */
        ...
        if (im->trueColor)
        {
            ...
            /* get pointer to the correct pixel row */
            pix = im->tpixels[y1];

            /* offset to starting pixel column and compute how many pixels to set */
            if (x1 <= x2)
            {
                pix += x1;
                npix = x2 - x1 + 1;
            ...
            if (im->alphaBlendingFlag && (gdTrueColorGetAlpha(color) != gdAlphaOpaque))
            {
                /* source color has alpha, so we have to blend */
                i = npix;
                do
                {
                    *pix++ = gdAlphaBlend (*pix, color);
                } while (--i>0);
            }
            else
            {
                
                /* source color has no alpha -- set directly */
                i = npix;
                do
                {
                    *pix++ = color;
...
static void gdImageSetAAPixelColor(gdImagePtr im, int x, int y, int color, int t)
{
    int a = gdTrueColorGetAlpha(color);
    int p = gdImageGetPixel(im,x,y);

    /* composite the color's actual alpha with the anti-alias
    fractional alpha */
    color = (color & 0x00FFFFFF) | ((127 - (((255 - t) * (127 - a) + 0x80) >> 8)) << 24);
    ...
    im->tpixels[y][x] = gdAlphaBlend(p, color);
}  
...
static void gdImageAALine (gdImagePtr im, int x1, int y1, int x2, int y2, int col)
{
    ...
    /* WCW: don't use clip_1d, as this causes dicontinuities
    along tile boundaries */

    /* WCW: ignore lines completely outside the image extent
    (allowing an additional buffer of 1 pixel) */
    if (x1 < -1 && x2 < -1)
        return;
    if (y1 < -1 && y2 < -1)
        return;
    if (x1 > im->cx2+1 && x2 > im->cx2+1)
        return;
    if (y1 > im->cy2+1 && y2 > im->cy2+1)
        return;

    if (x1 == x2 && y1 == y2)
    {
        /* TBB: allow setting points */
        if (x1 >= 0 && x1 <= im->cx2 && y1 >= 0 && y1 <= im->cy2)
            gdImageSetAAPixelColor(im, x1, y1, col, 0x0);
        return;
    }

    /* WCW: check if the line is fully within the image extent
    (allowing an additional buffer of 1 pixel) */
    within = (x1 >= 1 && x1 <= im->cx2-1 &&
        x2 >= 1 && x2 <= im->cx2-1 &&
        y1 >= 1 && y1 <= im->cy2-1 &&
        y2 >= 1 && y2 <= im->cy2-1);
    ...
    if (abs(dx) > abs(dy))
    {
        /* adjust points so we're iterating from x1 to x2, with x2 >= x1 */
        if (dx < 0)
        {
            tmp = x1;
            x1 = x2;
        ...
        if (within)
        {
            ...
            {
                tmp = y >> 16;
                gdImageSetAAPixelColor(im, x1, tmp  , col, ( y >> 8) & 0xFF);
                gdImageSetAAPixelColor(im, x1, tmp+1, col, (~y >> 8) & 0xFF);
    ...
    {
        /* adjust points so we're iterating from y1 to y2, with y2 >= y1 */
        if (dy < 0)
        {
            tmp = x1;
            x1 = x2;
        ...
        if (within)
        {
            ...
            {
                tmp = x >> 16;
                gdImageSetAAPixelColor(im, tmp  , y1, col, ( x >> 8) & 0xFF);
                gdImageSetAAPixelColor(im, tmp+1, y1, col, (~x >> 8) & 0xFF);

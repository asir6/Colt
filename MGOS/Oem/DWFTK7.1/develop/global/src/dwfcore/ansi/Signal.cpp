...
throw( DWFException )
{
    if (_bInit)
    {
        _DWFCORE_THROW( DWFIllegalStateException, L"Signal already initialized" );
    }
    ...
    {
        _DWFCORE_THROW( DWFUnexpectedException, L"Failed to initialize mutex" );
    }
    ...
    if (pthread_cond_init(&_tCondition, NULL) != 0)
    {
        ...
        _DWFCORE_THROW( DWFUnexpectedException, L"Failed to initialize signal" );
    }
...
throw( DWFException )
{
    if (_bInit == false)
    {
        _DWFCORE_THROW( DWFIllegalStateException, L"Signal not initialized" );
    }

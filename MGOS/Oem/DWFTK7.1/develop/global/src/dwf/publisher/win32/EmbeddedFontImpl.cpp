...
throw( DWFException )
{
    ...
    if ((_pFontOutputStream == NULL) && (_pFontFile == NULL))
    {
        if (_nCodePoints == 0)
        {
            _DWFCORE_THROW( DWFIOException, L"No data" );
        }
        ...
        {
            _DWFCORE_THROW( DWFMemoryException, L"Failed to allocate sparse character array" );
        }
        ...
        if (hDC == NULL)
        {
            _DWFCORE_THROW( DWFUnexpectedException, L"Failed to acquire a NULL DC" );
        }
        ...
        if (hObj == NULL)
        {
            ...
            _DWFCORE_THROW( DWFUnexpectedException, L"Failed to select font into DC" );
        }
        ...
        if (nMetricSize) 
        {
            ...
            if (apOTM.isNull())
            {
                _DWFCORE_THROW( DWFMemoryException, L"Failed to allocate buffer for metrics" );
            }
            ...
            {
                    //
                    // if the 1 bit is set, embedding is not allowed
                    //
                if (apOTM->otmfsType & 0x02)
                {
                    _DWFCORE_THROW( DWFIllegalStateException, L"No license to embed font" );
                }
            }
        }
...
throw ( DWFException )
{
    ...
    {
        _DWFCORE_THROW( DWFNullPointerException, L"No font resource available for loading" );
    }
    ...
    _DWFCORE_SWPRINTF( zPostfix, 9, L"%0.4x", knCount++ );
    ...
    bool bCompliant = true;
    {
        ...
        {
            _DWFCORE_THROW( DWFIOException, L"Error reading from font stream" );
        }
        ...
        {
            _DWFCORE_THROW( DWFIOException, L"Error writing to buffer" );
        }
        ...
        if (_pFontOutputStream->write(&nReadBytes, 2) != (size_t)2)
        {
            ...
            // first name 
            //
        if (_pFontInputStream->read(zName, nReadBytes + 2) != (size_t)(nReadBytes + 2))
        {
            _DWFCORE_THROW( DWFIOException, L"Error reading from font stream" );
        }
            ...
            // cache these bytes
            //
        if (_pFontOutputStream->write(zName, nReadBytes + 2) != (size_t)(nReadBytes + 2))
        {
            _DWFCORE_THROW( DWFIOException, L"Error writing to buffer" );
        }
        ...
        if (_pFontOutputStream->write(zBuffer, nReadBytes + 2) != (size_t)(nReadBytes + 2))
        {
            ...
            //
            // compliance test
            // TODO: we need to deal with the asian font case where the below test fails on a compliant font
            //
    ...
    if ((bLoaded == false) && bForceRename)
    {
        ...
        _pFontInputStream = _pFontResource->getInputStream();
                ...
                //
                // make another attempt to load the font 
                //
        if ((*_kfpTTLoadEmbeddedFont)(&_hLoadedFont, 
                                       _nRequest, 
                                       &nPrivStatus,
                                        ...
                                        NULL))
        {
            ...
            // the attempt failed
            //
            _DWFCORE_THROW( DWFIllegalStateException, L"Failed to load the embedded font" );
        }
    }
...
DWFEmbeddedFontImpl_Win32::_loadFontLibrary( void )
{
    if (_khLib == NULL)
    {
        ...
        if (_khLib == NULL)
        {
            _DWFCORE_THROW( DWFUnexpectedException, L"Failed to load TrueType embedding library" );
        }
        ...
        _kfpTTEmbedFont = (fpTTEmbedFont)::GetProcAddress( _khLib, "TTEmbedFont" );
        if (_kfpTTEmbedFont == NULL)
        {
            _DWFCORE_THROW( DWFUnexpectedException, L"Failed to locate TTEmbedFont API" );
        }

        _kfpTTLoadEmbeddedFont = (fpTTLoadEmbeddedFont)::GetProcAddress( _khLib, "TTLoadEmbeddedFont" );
        if (_kfpTTLoadEmbeddedFont == NULL)
        {
            _DWFCORE_THROW( DWFUnexpectedException, L"Failed to locate TTLoadEmbeddedFont API" );
        }

        _kfpTTDeleteEmbeddedFont = (fpTTDeleteEmbeddedFont)::GetProcAddress( _khLib, "TTDeleteEmbeddedFont" );
        if (_kfpTTLoadEmbeddedFont == NULL)
        {
            _DWFCORE_THROW( DWFUnexpectedException, L"Failed to locate TTDeleteEmbeddedFont API" );
        }
    }
        //

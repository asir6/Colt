    ...
    switch (_nElementDepth)
    {
                ...
                for(; ppAttributeList[iAttrib]; iAttrib += 2)
                {
                    ...
                    else if (DWFCORE_COMPARE_ASCII_STRINGS(pAttrib, DWFXML::kzAttribute_Refs) == 0)
                    {
                        ...
                        while (pToken)
                        {
                            ...
                            {
                                _pCurrentRefs = DWFCORE_ALLOC_OBJECT( vector<DWFString> );
                            }
                            if (_pCurrentRefs)
                            {
        ...
        case 2:
        {
                ...
                (DWFCORE_COMPARE_ASCII_STRINGS(zName, DWFXML::kzElement_Property) == 0))
            {
                ...
                {
                    DWFProperty* pProperty = _pElementBuilder->buildProperty( ppAttributeList );
                    if (pProperty)
                    {
        ...
        case 4:
        {
            if (_pCurrentObject && (DWFCORE_COMPARE_ASCII_STRINGS(zName, DWFXML::kzElement_Property) == 0))
            {
                ...
                //
                _pCurrentObject->addProperty( _pElementBuilder->buildProperty(ppAttributeList), true ); 
            }
            break;
        }
        default:

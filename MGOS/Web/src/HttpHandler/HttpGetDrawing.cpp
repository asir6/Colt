...
HTTP_IMPLEMENT_CREATE_OBJECT(MgHttpGetDrawing)
...
/// </returns>
void MgHttpGetDrawing::Execute(MgHttpResponse& hResponse)
{
    ...
    // call the C++ API
    Ptr<MgByteReader> byteReaderResult = service->GetDrawing(&resId);
    hResult->SetResultObject(byteReaderResult, byteReaderResult->GetMimeType());

    MG_HTTP_HANDLER_CATCH_AND_THROW_EX(L"MgHttpGetDrawing.Execute")
}

...
/*! BBINFILETK_API resolves to whatever special keywords are required to make a function or class
    part of the externally supported API */
#ifdef _MSC_VER
    #ifndef BSTREAM_STATIC_LIB
        #ifdef BBINFILETK_EXPORTS
            #define BBINFILETK_API  __declspec (dllexport)
        #else
            #define BBINFILETK_API  __declspec (dllimport)
        #endif
    #else
...
*/
#define TK_File_Format_Version       1200   // 12.00
...
/*! ID_Key is just a generic long integer by which it is assumed that the graphics system
...
/*! \enum TK_File_Read_Option Options which control how the HOOPS Stream File is read */

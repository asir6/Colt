...
throw()
{
    ...
    if (piResource)
    {
              ...
              piResource->next())
        {
                ...
                // delete if owned
                //
            if (piResource->value()->owner() == this)
            {
                DWFCORE_FREE_OBJECT( piResource->value() );
            }
        }
...
throw( DWFException )
{
    ...
    if (ppResource)
    {
        ...
        {
            _DWFCORE_THROW( DWFTypeMismatchException, L"Resource with matching HREF was found but was not the same object" );
        }
    }
    else
    {
...
throw( DWFException )
{
    ...
    {
        _DWFCORE_THROW( DWFDoesNotExistException, L"Resource not found" );
    }
}
...
throw( DWFException )
{
    ...
    if (iFirst != _oResourcesByRole.end())
    {
        ...
        {
            _DWFCORE_THROW( DWFMemoryException, L"Failed to allocate memory" );
        }
...
throw( DWFException )
{
    ...
    if (pResource)
    {
        ...
        if (pResource->href().bytes() == 0)
        {
            ...
            // TODO: is this OK?
            //
            pResource->setHRef( _oUUID.next(true) );
        }
        ...
        rSerializer.startElement( DWFXML::kzElement_TOC, DWFXML::kzNamespace_DWF );
        {
            ...
            for (; iResource != _pContainer->_oResourcesByRole.end(); iResource++)
            {
                ...
                if (pResource->role() != DWFXML::kzRole_Descriptor)
                {
                    ...
                    //
                    pResource->setPublishedIdentity( _pContainer->_zName, rSerializer.nextUUID(true) );

...
static char const * opcode_string[256] = {
    ...
    "Heuristics          ", "File_Info           ", "Line Style          ", "Renumber_Key_Global ", 
...
TK_Status TK_Default::Read (BStreamFileToolkit & tk) alter {
    ...
    auto        unsigned char   opcode = '\0';
    auto        unsigned int    sequence;
...
TK_Status TK_Comment::Read (BStreamFileToolkit & tk) alter {
    ...
    while (1) {
        ...
        if (m_progress == m_length) {
            auto    char *      old_buffer = m_comment;
...
TK_Status TK_Comment::Execute (BStreamFileToolkit & tk) alter {
    if (!tk.m_header_comment_seen) {
        ...
        auto    char const *    cp = &m_comment[7];
        auto    int             version = 0;
...
TK_Status TK_Open_Segment::Write (BStreamFileToolkit & tk) alter {
    ...
    switch (m_stage) {
        ...
        case 1: {
            ...
            auto        unsigned char       byte = (unsigned char)m_length;
...
TK_Status TK_Tag::Execute (BStreamFileToolkit & tk) alter {
    ...
    switch (Opcode()) {
        case TKE_Tag: {
            auto        int             index;
            auto        char            buffer[32];
            auto        bool            logging = tk.GetLogging() &&
...
TK_Status TK_Dictionary::Write (BStreamFileToolkit & tk) alter {
    ...
    switch (m_stage) {
        ...
        case 8: {
            m_format = (unsigned char)tk.GetDictionaryFormat();
            if (m_format == 1) {
                auto        int         offset = tk.GetFirstPause();
        ...
        case 9: {
            ...
            auto        int         offset = tk.GetDictionaryOffset();
...
TK_Status TK_Dictionary::Read (BStreamFileToolkit & tk) alter {
    ...
    switch (m_stage) {
        ...
        case 2: {
            if (m_format >= 2) {
                auto        int *       table = new int [m_progress];
                register    int         i;
        ...
        case 5: {
            ...
            while (m_placeholder < m_number_of_items) {
                switch (m_progress) {
                    ...
                    default: {
                        auto    int         num_offsets = m_format == 0 ? 1 : 2;
...
TK_Status TK_Instance::Write (BStreamFileToolkit & tk) alter {
    ...
    switch (m_stage) {
        ...
        case 5: {
            auto        unsigned char       byte = (unsigned char)m_options;
            if ((status = PutData (tk, byte)) != TK_Normal)
                return status;
            m_stage++;
        }   nobreak;
...
TK_Status HT_NURBS_Trim::write_collection (BStreamFileToolkit & tk) alter {
    ...
    switch( m_substage ) {
        ...
        case 1: {
            auto        unsigned char       end = NS_TRIM_END;
...
TK_Status TK_Cutting_Plane::Write (BStreamFileToolkit & tk) alter {
    ...
    switch (m_stage) {
        ...
        case 1: {
            if ((status = PutData (tk, m_plane, 4)) != TK_Normal)
                return status;
            m_stage++;
        }   nobreak;
...
TK_Status TK_Cutting_Plane::Read (BStreamFileToolkit & tk) alter {
    ...
    switch (m_stage) {
        case 0: {
            if ((status = GetData (tk, m_plane, 4)) != TK_Normal)
                return status;
            m_stage = -1;
        }   break;
...
#ifndef BSTREAM_READONLY
    auto        TK_Status       status = TK_Normal;
    auto        bool            do_attributes = (tk.GetTargetVersion() >= 805);
        ...
        case 4: {
            if (tk.GetTargetVersion() > 600) {
                if (m_length > 65535) {
                    ...
                    auto    unsigned short      word = (unsigned short)m_length;
...
TK_Status TK_Font::Write (BStreamFileToolkit & tk) alter {
    ...
    switch (m_stage) {
        ...
        case 5: {
            auto        unsigned char       byte = (unsigned char)m_lookup_length;
...
static int const image_bytes_per_pixel[] = {
    1, 2, 3, 4, 4
};
...
TK_Status TK_Image::compress_image (BStreamFileToolkit & tk) {
#ifndef BSTREAM_READONLY
	...
	cinfo.client_data = null; 
        ...
        if (initial_size < 4096)
            initial_size = 4096;
        m_work_area.Resize (initial_size);
        ...
        if (m_work_area.Size() - m_work_area.Used() < 256)
            m_work_area.Expand (256);
        ...
        auto        char const *        input = m_bytes;
        auto        unsigned int        total = m_size[0] * m_size[1] * image_bytes_per_pixel[(int)m_format];
        auto        char const *        end = input + total;
        auto        char const *        current = input;
        auto        char const *        run = input;
        auto        char alter *        output;

        m_work_area.Resize (total >> 2);   
        ...
        while (input != end) {
            ...
            if (current == end || current - run >= 4) {
                while (input != run) {
                    auto        int     count = (int)(run - input);

                    if (count > 127)
                        count = 127;    

                    if (m_work_area.Used()+count+1 > m_work_area.Size())
                        m_work_area.Expand (4096);
                    output = (char alter *)m_work_area.Buffer() + m_work_area.Used();

                    m_work_area.Used() += count+1;   
                    *output++ = (char)count;
                    while (count-- > 0)
                        *output++ = *input++;
                }
            ...
            if (current - run >= 4) {
                ...
                while (run != current) {
                    auto        int     count = (int)(current - run);
                    ...
                    if (m_work_area.Used()+2 > m_work_area.Size())
                        m_work_area.Expand (4096);
                    output = (char alter *)m_work_area.Buffer() + m_work_area.Used();
...
TK_Status TK_Image::decompress_image (BStreamFileToolkit & tk) {
	...
	cinfo.client_data = null;
        ...
        auto        char const *        input = (char const *)m_work_area.Buffer();
        auto        int                 remaining = m_work_area.Used();
        auto        char alter *        output = m_bytes;
        auto        unsigned int        total = m_size[0] * m_size[1] * image_bytes_per_pixel[(int)m_format];

        while (remaining-- > 0) {
            auto    int                 count = *input++;
...
TK_Status TK_Image::Write (BStreamFileToolkit & tk) alter {
    ...
    switch (m_stage) {
        ...
        case 2: {
            auto            unsigned char       byte = (unsigned char)(m_format | m_options);
        ...
        case 7: {
	    if (m_compression == TKO_Compression_Reference) {
		auto	    unsigned short	word = (unsigned short)m_reference_length;
                ...
                if ((status = PutData (tk, m_work_area.Used())) != TK_Normal)
                    return status;
            }
            m_stage++;
        }   nobreak;
        ...
        case 8: {
            if (m_compression == TKO_Compression_Reference) {
                ...
                auto        int     total = m_size[0] * m_size[1] * image_bytes_per_pixel[(int)m_format];

                if ((status = PutData (tk, m_bytes, total)) != TK_Normal)
                    return status;
            }
            else {
                if ((status = PutData (tk, m_work_area.Buffer(), m_work_area.Used())) != TK_Normal)
                    return status;
            }
            m_stage++;
        }   nobreak;
    ...
    switch (m_stage) {
        ...
        case 7: {
            if (m_compression == TKO_Compression_Reference) {
                ...
                if ((status = GetData (tk, m_work_area.Buffer(), m_work_area.Size())) != TK_Normal)
                    return status;
                m_work_area.Used() = m_work_area.Size();
...
void TK_Image::set_data (int count, char const * bytes, unsigned char data_format) alter {
    ...
    if (bytes != null) {
        ...
        if ((m_bytes_format = data_format) == TKO_Compression_JPEG) {
            m_work_area.Resize (count);
            memcpy (m_work_area.Buffer(), bytes, count*sizeof(char));
            m_work_area.Used() = count;
            read_jpeg_header();
        }
    }
}
...
TK_Status TK_Thumbnail::Write (BStreamFileToolkit & tk) alter {
    ...
    switch (m_stage) {
        ...
        case 3: {
            auto        int     total = m_size[0] * m_size[1] * thumbnail_bytes_per_pixel[(int)m_format];
...
TK_Status TK_Color::Write (BStreamFileToolkit & tk) alter {
    ...
    switch (m_stage) {
        ...
        case 1: {
            auto        unsigned char       byte = (unsigned char)(m_mask & 0x000000FF);
        ...
        case 2: {
            if ((m_mask & TKO_Geo_Extended) != 0) {
                auto    unsigned char       byte = (unsigned char)((m_mask>>8) & 0x000000FF);
        ...
        case 3: {
            if ((m_mask & TKO_Geo_Extended_Colors) != 0) {
                auto    unsigned char       byte = (unsigned char)((m_mask>>16) & 0x000000FF);
        ...
        case 4: {
            auto        unsigned char       byte = (unsigned char)(m_channels & 0x000000FF);
        ...
        case 5: {
            if ((m_channels & (1 << TKO_Channel_Extended)) != 0) {
                auto    unsigned char       byte = (unsigned char)((m_channels>>8) & 0x000000FF);
        ...
        case 6: {
            if ((m_channels & (1 << TKO_Channel_Diffuse)) != 0) {
                ...
                if (m_diffuse.m_name != null)
                    byte = (unsigned char) strlen (m_diffuse.m_name);
        ...
        case 8: {
            if ((m_channels & (1 << TKO_Channel_Specular)) != 0) {
                ...
                if (m_specular.m_name != null)
                    byte = (unsigned char) strlen (m_specular.m_name);
        ...
        case 10: {
            if ((m_channels & (1 << TKO_Channel_Mirror)) != 0) {
                ...
                if (m_mirror.m_name != null)
                    byte = (unsigned char) strlen (m_mirror.m_name);
        ...
        case 12: {
            if ((m_channels & (1 << TKO_Channel_Transmission)) != 0) {
                ...
                if (m_transmission.m_name != null)
                    byte = (unsigned char) strlen (m_transmission.m_name);
        ...
        case 14: {
            if ((m_channels & (1 << TKO_Channel_Emission)) != 0) {
                ...
                if (m_emission.m_name != null)
                    byte = (unsigned char) strlen (m_emission.m_name);
        ...
        case 18: {
            if ((m_channels & (1 << TKO_Channel_Environment)) != 0) {
                auto        unsigned char       byte = (unsigned char) strlen (m_environment.m_name);
        ...
        case 20: {
            if ((m_channels & (1 << TKO_Channel_Bump)) != 0) {
                auto        unsigned char       byte = (unsigned char) strlen (m_bump.m_name);
...
void TK_Color::set_channel_name (channel alter & c, int length, int channel) alter {
    ::delete [] c.m_name;
...
void TK_Color::set_channel_name (channel alter & c, char const * name, int channel) alter {
    if (name != null) {
        set_channel_name (c, (int) strlen (name), channel);
        ...
        case 4: {
            switch (Opcode()) {
                case TKE_Color_By_Index: {
                    auto        unsigned char       byte = (unsigned char)m_index;
                ...
                case TKE_Color_By_Index_16: {
                    auto        unsigned short      word = (unsigned short)m_index;
...
TK_Status TK_Color_Map::Write (BStreamFileToolkit & tk) alter {
    ...
    switch (m_stage) {
        ...
        case 3: {
            auto        unsigned char       values[3*256];
    ...
    switch (m_stage) {
        ...
        case 1: {
            if (m_length > 0) {
                ...
                auto        short               word = (short)m_length;
...
#ifndef BSTREAM_READONLY
    auto        TK_Status       status = TK_Normal;
    auto        int             mask = m_mask[0], emask = m_mask[1];
    ...
    switch (m_stage) {
        ...
        case 2: {
            if ((mask & TKO_Rendo_Extended) != 0) {
                if ((status = PutData (tk, m_mask[1])) != TK_Normal)
                    return status;
            }
            m_stage++;
        }   nobreak;
        ...
        case 21: {
            if (emask &&
                (m_mask[1] & m_value[1] & TKO_Rendo_Transparency_Style) != 0) {
        ...
        case 22: {
            if (emask &&
                (m_mask[1] & m_value[1] & TKO_Rendo_Cut_Geometry) != 0) {
		switch (m_progress) {
		    case 0: {
        ...
        case 23: {
            if (emask &&
                (m_mask[1] & m_value[1] & TKO_Rendo_Depth_Range) != 0) {
        ...
        case 24: {
            if (emask &&
                (m_mask[1] & m_value[1] & TKO_Rendo_Mask_Transform) != 0) {
        ...
        case 25: {
            if (emask &&
                (m_mask[1] & m_value[1] & TKO_Rendo_Image_Scale) != 0) {
		...
		switch (m_progress) {
		    case 0: {
			auto	    unsigned char	byte = (unsigned char)(smask & 0x00FF);
			...
			if ((smask & TKO_Simple_Shadow_Extended) != 0) {
			    auto	    unsigned char	byte = (unsigned char)((smask >> 8) & 0x00FF);
...
TK_Status TK_Rendering_Options::Read (BStreamFileToolkit & tk) alter {
    ...
    switch (m_stage) {
        ...
        case 21: {
            if ((m_mask[1] & m_value[1] & TKO_Rendo_Transparency_Style) != 0) {
...
TK_Status TK_Heuristics::Write (BStreamFileToolkit & tk) alter {
    ...
    switch (m_stage) {
        ...
        case 2: {
            if (tk.GetTargetVersion() >= 705 && (m_mask & TKO_Heuristic_Extended)) {
        ...
        case 6: {
            if ((m_mask & m_value & TKO_Heuristic_Internal_Select_Limit) != 0) {
                if ((status = PutData (tk, m_internal)) != TK_Normal)
                    return status;
            }
            m_stage++;
        }   nobreak;
        ...
        case 8: {
            if (tk.GetTargetVersion() >= 705 && (m_mask & TKO_Heuristic_Culling)) {
                if ((status = PutData (tk, m_culling)) != TK_Normal)
                    return status;
            }
            m_stage = -1;
        }   break;
...
TK_Status TK_Heuristics::Read (BStreamFileToolkit & tk) alter {
    ...
    switch (m_stage) {
        ...
        case 5: {
            if ((m_mask & m_value & TKO_Heuristic_Internal_Select_Limit) != 0) {
                if ((status = GetData (tk, m_internal)) != TK_Normal)
                    return status;
            }
            m_stage++;
        }   nobreak;
        ...
        case 7: {
            if ((m_mask & m_value & TKO_Heuristic_Culling) != 0) {
                if ((status = GetData (tk, m_culling)) != TK_Normal)
                    return status;
            }
            m_stage = -1;
        }   break;
...
TK_Status TK_Visibility::Write (BStreamFileToolkit & tk) alter {
    ...
    switch (m_stage) {
        ...
        case 2: {
            auto        unsigned char       byte = (unsigned char) (m_value & 0x000000FF);
        ...
        case 3: {
            if ((m_mask & TKO_Geo_Extended) != 0) {
                auto    unsigned short      word = (unsigned short) ((m_mask>>8) & 0x0000FFFF);
        ...
        case 4: {
            if ((m_mask & TKO_Geo_Extended) != 0) {
                auto    unsigned short      word = (unsigned short) ((m_value>>8) & 0x0000FFFF);
...
TK_Status TK_Selectability::Write (BStreamFileToolkit & tk) alter {
    ...
    switch (m_stage) {
        ...
        case 1: {
            if ((m_mask & TKO_Geo_Extended) != 0) {
                auto        unsigned short      word = (unsigned short)(m_mask & 0x0000FFFF);
        ...
        case 2: {
            if ((m_mask & TKO_Geo_Extended) != 0) {
                auto        unsigned short      word = (unsigned short)(m_down & 0x0000FFFF);

                if ((status = PutData (tk, word)) != TK_Normal)
                    return status;
            }
            else {
                auto        unsigned char       byte = (unsigned char)(m_down & 0x000000FF);
        ...
        case 3: {
            if ((m_mask & TKO_Geo_Extended) != 0) {
                auto        unsigned short      word = (unsigned short)(m_up & 0x0000FFFF);

                if ((status = PutData (tk, word)) != TK_Normal)
                    return status;
            }
            else {
                auto        unsigned char       byte = (unsigned char)(m_up & 0x000000FF);
        ...
        case 4: {
            if ((m_mask & TKO_Geo_Extended) != 0) {
                auto        unsigned short      word = (unsigned short)(m_move_down & 0x0000FFFF);

                if ((status = PutData (tk, word)) != TK_Normal)
                    return status;
            }
            else {
                auto        unsigned char       byte = (unsigned char)(m_move_down & 0x000000FF);
        ...
        case 5: {
            if ((m_mask & TKO_Geo_Extended) != 0) {
                auto        unsigned short      word = (unsigned short)(m_move_up & 0x0000FFFF);

                if ((status = PutData (tk, word)) != TK_Normal)
                    return status;
            }
            else {
                auto        unsigned char       byte = (unsigned char)(m_move_up & 0x000000FF);
        ...
        case 6: {
            if ((m_mask & TKO_Geo_Extended) != 0) {
                auto        unsigned short      word = (unsigned short)(m_invisible & 0x0000FFFF);

                if ((status = PutData (tk, word)) != TK_Normal)
                    return status;
            }
            else {
                auto        unsigned char       byte = (unsigned char)(m_invisible & 0x000000FF);
...
TK_Status TK_Named::Write (BStreamFileToolkit & tk) alter {
    ...
    switch (m_stage) {
        ...
        case 1: {
	    if (m_name_length > 0) {
		auto	unsigned char	    byte = (unsigned char)0xFF;

		if ((status = PutData (tk, byte)) != TK_Normal)
		    return status;
	    }
	    else if (Opcode() != TKE_Marker_Symbol) {
		auto	unsigned short	    word = (unsigned char)m_index;
    ...
    switch (m_stage) {
        ...
        case 3: {
            while (m_progress < m_count) {
                auto    int     value = (int)m_values[m_progress];
...
TK_Status TK_Camera::Write (BStreamFileToolkit & tk) alter {
    ...
    switch (m_stage) {
        ...
        case 6: {
            if (Opcode() == TKE_View) {
                auto        unsigned char       length = (unsigned char)m_length;
...
TK_Status TK_Text_Font::Write (BStreamFileToolkit & tk) alter {
    ...
    switch (m_stage) {
        ...
        case 4: {
            if ((m_mask & TKO_Font_Extended) != 0) {
                auto    unsigned char       byte = (unsigned char)((m_value>>8) & 0x000000FF);
        ...
        case 5: {
            if ((m_mask & m_value & TKO_Font_Names) != 0) {
                ...
                auto        unsigned char       byte = (unsigned char)m_names_length;
                if ((status = PutData (tk, byte)) != TK_Normal)
                    return status;
            }
            m_stage++;
...
TK_Status TK_Texture::Write (BStreamFileToolkit & tk) alter {
    ...
    switch (m_stage) {
        ...
        case 3: {
            ...
            auto        unsigned char       byte = (unsigned char)m_image_length;
            if ((status = PutData (tk, byte)) != TK_Normal)
                return status;
            m_stage++;
        }   nobreak;
        ...
        case 5: {
	    auto	unsigned short	    word = (unsigned short)flags;
		...
		auto	unsigned short	    word = (unsigned short)(flags >> 16);
        ...
        case 10: {
            if ((flags & TKO_Texture_Decimation) != 0)
                if ((status = PutData (tk, m_decimation)) != TK_Normal)
                    return status;
            m_stage++;
        }   nobreak;
        ...
        case 18: {
            if ((flags & TKO_Texture_Transform) != 0) {
                auto        unsigned char       byte = (unsigned char) strlen (m_transform);
...
TK_Status TK_Glyph_Definition::Write (BStreamFileToolkit & tk) alter {
    ...
    switch (m_stage) {
        ...
        case 3: {
            ...
            auto        unsigned short      word = (unsigned short)m_size;
...
TK_Status TK_Line_Style::Write (BStreamFileToolkit & tk) alter {
    ...
    switch (m_stage) {
        ...
        case 3: {
            ...
            auto        unsigned short      word = (unsigned short)m_definition_length;
...
void TK_XML::AppendXML (int count, char const * bytes) alter {
    auto        char *              old_data = m_data;
    auto        int                 old_size = m_size;

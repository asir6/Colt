...
local int points_are_collapsible (
    auto        HT_Point const *    pts,
    auto        HT_Vector const *   norms,
    auto        int                 parameter_width,
    auto        float const *       parameters,
    auto        int                 a,
    auto        int                 b,
    auto        double              tolerance_squared,
    auto        double              normal_distance_squared,
    auto        double              parameter_distance_squared) {
    ...
    if (parameters != NULL) {
        auto        int             pa = a * parameter_width;
        auto        int             pb = b * parameter_width;
        register    int             i;
...
void HU_OptimizeShell (
    auto        int                     in_point_count,
    auto        HT_Point const *        in_points,
    auto        HT_Vector const *       in_normals,         // null if not interested
    auto        int                     in_parameter_width,
    auto        float const *           in_parameters,      // null if not interested
    auto        int                     in_face_list_length,
    auto        int const *             in_face_list,

    auto        HT_Collapse_Check_Function  collapsible,
    auto        double                  tolerance,
    auto        double                  normal_tolerance,
    auto        double                  parameter_tolerance,
    auto        int                     orphan_elimination,

    auto        int *                   out_point_count,
    auto        HT_Point *              out_points,
    auto        int *                   out_face_list_length,
    auto        int *                   out_face_list,
    auto        int *                   vertex_mapping,     // null if not interested
    auto        int *                   face_mapping) {     // null if not interested
    auto        Vertex                  *vertices;
    auto        int *                   tmp_vertex_mapping = null;
    auto        float                   hood;
    auto        double                  tolerance_squared;  // double because blindly passed
    auto        double                  normal_distance_squared;
    auto        double                  parameter_distance_squared;
    ...
    // Find unique points
    {
        ...
        register        Vertex          *vp = vertices;
        auto            Vertex          *ve = vp + in_point_count;
        auto            int             unique_point_count = 0;
    ...
    if (out_face_list_length != null) {
        if (in_face_list_length > 0) {
            ...
            do {
                register    int const       *local_end;
                register    int             prev_index;
                auto        int             *face_length_ptr;
                auto        int             face_length;
                auto        int             is_hole;

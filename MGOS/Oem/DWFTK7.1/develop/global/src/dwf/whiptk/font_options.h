...
public:
    ...
    WT_Font_Option_Font_Name(WT_String name)
        : m_font_name (name)
    { }
    //@}
    ...
    WT_String const &   name() const                                            {   return m_font_name;   }
    /// Sets the font name.
    void                set(WT_String const & name)                             {   m_font_name = name;   }
    /// Returns WD_True if the given font name is equal to this one.
    WT_Boolean          operator==(WT_Font_Option_Font_Name const & nm) const   {   return (m_font_name == nm.m_font_name); }
    /// Returns WD_True if the given font name is NOT equal to this one.
    WT_Boolean          operator!=(WT_Font_Option_Font_Name const & nm) const   {   return (m_font_name != nm.m_font_name); }
    /// Returns the font name.
    operator WT_String const & () const                     {   return m_font_name;     }
    //@}
...
public:
    ...
    WT_Font_Option_Spacing()
        : m_spacing (1024) // Pulling a rabit out of our hat!  Shouldn't use hard coded constants.
    { }

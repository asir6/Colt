...
_DWFCORE_API
DWFUUID::DWFUUID( const DWFUUID& uuid )
throw()
       : _pImpl( NULL )
{
    _pImpl = uuid._pImpl->clone();
}
...
_DWFCORE_API
DWFUUID&
DWFUUID::operator=( const DWFUUID& uuid )
throw()
{
    _pImpl = uuid._pImpl->clone();
    return *this;
...
throw( DWFException )
{
    if (_pImpl == NULL)
    {
        ...
        if (_pImpl == NULL)
        {
            _DWFCORE_THROW( DWFMemoryException, L"Failed to initialize object" );
        }
    }

...
void Internal_Data_Accumulator::save (void) alter {
    auto        int             needed = m_failed_size;
    auto        char alter *    old_buffer = null;
...
static float determinant (float const * points, int const * indices) {
    auto        float const *   p0 = &points[3*indices[0]];
    auto        float const *   p1 = &points[3*indices[1]];
    auto        float const *   p2 = &points[3*indices[2]];
    auto        float const *   p3 = &points[3*indices[3]];
    auto        float           v0[3], v1[3], v2[3];
...
static bool invert_matrix (
    ...
    auto        float       a10_21_11_20 = a[ 4]*a[ 9] - a[ 5]*a[ 8],
    ...
    auto        float       det = a[0]*p0 - a[1]*p1 + a[2]*p2 - a[3]*p3;
...
bool Recorded_Instance::generate_basis (int count, float const * points) alter {
    ...
    if (count > 0) {
        ...
        if (find_basis_points (count, points, m_basis_indices) >= epsilon) {
            auto    int             b0 = 3*m_basis_indices[0];
            auto    int             b1 = 3*m_basis_indices[1];
            auto    int             b2 = 3*m_basis_indices[2];
            auto    int             b3 = 3*m_basis_indices[3];
            auto    float           matrix[16];
...
BStreamFileToolkit::BStreamFileToolkit() {
    ...
    SetOpcodeHandler (W3DE_Image,               new W3D_Image);
}
...
void BStreamFileToolkit::RecordPause (int offset) alter {
    if (m_num_pauses >= m_pause_table_size) {
        auto        int *       old_table = m_pause_table;
...
TK_Status BStreamFileToolkit::LocateDictionary (void) alter {
    auto        FILE *          fp = (FILE *)m_file;
    auto        char            block[TK_DEFAULT_BUFFER_SIZE];
    auto        int             amount;
...
TK_Status BStreamFileToolkit::LocateEntity (ID_Key key, int variant) alter {
    auto        int             offset, length;
    auto        char const *    oldfile = GetCurrentFile();
    auto        char const *    newfile;
    auto        TK_Status       status = TK_Normal;
...
TK_Status BStreamFileToolkit::ParseBuffer (char const * b, int s, TK_Status mode) alter {
    auto    TK_Status                   status = TK_Normal;
    auto    TK_Status                   executed;
    auto    Internal_Segment_List *     current_active = null;
    auto    ID_Key                      current_key = -1;
    auto    bool                        local_open = false;
    ...
    forever {
        ...
        if (mode == TK_Single && m_nesting_level == 0) {
            status = TK_Complete;
            break;
        }
    }
...
TK_Status BStreamFileToolkit::sort_revisit( ) {
    auto Internal_Revisit_Item *ptr;
    auto Internal_Revisit_Item **array;
    auto int i = 0, count = 0;
...
void BStreamFileToolkit::AddSegment (ID_Key key) alter {
    auto    Internal_Segment_List *         newseg;
...
ID_Key BStreamFileToolkit::RemoveSegment (void) alter {
    auto    Internal_Segment_List *         oldseg;
    auto    ID_Key                          old_key;
...
void BStreamFileToolkit::remember_segment (ID_Key key) alter {
    if (key == -1 || find_segment (key))
        return;

    m_visited_segments.add_key (key);
}

bool BStreamFileToolkit::find_segment (ID_Key key) const {
    return  m_visited_segments.find_key (key) == TK_Normal;
}
...
TK_Status BStreamFileToolkit::revisit (unsigned char opcode, float priority, int lod) alter {
    auto    Internal_Revisit_Item *     item = new Internal_Revisit_Item;
...
void BStreamFileToolkit::AddExternalReference (char const * ref, ID_Key context) alter {
    auto    Internal_ExRef_List *   exref = new Internal_ExRef_List (ref, context);
...
bool BStreamFileToolkit::NextExternalReference (void) alter {
    auto    Internal_ExRef_List *   exref;
...
inline int hash (int val1, int val2, int val3) {
    register    int     tmp;
...
void BStreamFileToolkit::record_instance (ID_Key key, int variant,
    ...
    register    int                         index = hash (val1, val2, val3);
    auto        Recorded_Instance *         instance = new Recorded_Instance (key, variant, object->Opcode(),
                                                                              val1, val2, val3);
...
bool BStreamFileToolkit::find_instance (BBaseOpcodeHandler * object, int val1, int val2, int val3) const {
    register    int                         index = hash (val1, val2, val3);
    auto        Recorded_Instance *         instance;
...
TK_Status Internal_Translator::add_pair (int index, ID_Key key) alter {
    if (index >= m_size) {
        auto        Index_Key_Pair *    old_pairs = m_pairs;
...
TK_Status Internal_Translator::index_to_key (int index, ID_Key alter & key) const {
    ...
    if (index >= m_size)
        return TK_NotFound;
    if (m_pairs[index].m_index != index)
        return TK_NotFound;
...
TK_Status BStreamFileToolkit::GetOffset (
    auto        ID_Key                  key,
    auto        int                     variant,
    auto        int alter &             offset,
    auto        int alter &             length,
    auto        char const * alter &    filename) const {
    auto        TK_Status               status = TK_Normal;
    auto        int                     index;
    register    int                     i;
...
TK_Status BStreamFileToolkit::Read_Stream_File () {
    auto        char                    block[TK_DEFAULT_BUFFER_SIZE];
    auto        int                     limit = TK_DEFAULT_BUFFER_SIZE;
    auto        TK_Status               status = TK_Normal;
    auto        bool                    version_mismatch = false;
    auto        int                     flags;
    auto        unsigned long           file_size = 0;
    auto        unsigned long           processed = 0;
    auto        TK_Progress_Callback    callback = 0;
    auto        void *                  value = 0;
    auto        bool                    first_file = true;
    auto        ID_Key                  context = -1;
	auto		unsigned short const *	wfilename = m_wfilename;
	auto		char const *			filename = m_filename;

    if (GetLogging()) {
        auto    char const *            logfile = GetLogFile();
        ...
        do {
            ...
            if (status == TK_Version) {
                if (first_file) {
                    ...
                    Restart();
                    SetReadFlags (flags | TK_Ignore_Version);
                    status = ParseBuffer (block, amount);
                }
                else
                    break;

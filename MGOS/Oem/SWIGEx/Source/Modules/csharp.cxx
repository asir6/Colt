...
char cvsroot_csharp_cxx[] = "$Header: /cvsroot/SWIG/Source/Modules/csharp.cxx,v 1.33 2003/12/13 00:13:10 cheetah Exp $";
...
class CSHARP : public Language {
    ...
    for (int i = 1; i < argc; i++) {
      if (argv[i]) {
        if (strcmp(argv[i],"-package") == 0) {
          ...
          if (argv[i+1]) {
            namespaceName = NewString("");
            Printf(namespaceName, argv[i+1]);
            Swig_mark_arg(i);
            Swig_mark_arg(i+1);
            i++;
          } else {
  ...
  virtual int top(Node *n) {
    ...
    // Generate the intermediary class
    {
      ...
      Replaceall(imclass_class_code,"$dllname", unmanagedDllName);
  ...
  virtual int functionWrapper(Node *n) {
    ...
    bool wrappingConstant = Cmp(nodeType(n), "constant") == 0;
    if (wrappingConstant) {
  ...
  void emitProxyClassDefAndCPPCasts(Node *n) {
    ...
    {
        //root exception. emit modified class declaration
        ...
        Printv(proxy_class_def,
            ...
            "  ",
            typemapLookup("csptrconstructormodifiers", typemap_lookup_type, WARN_CSHARP_TYPEMAP_PTRCONSTMOD_UNDEF), // pointer constructor modifiers
            " $csclassname(IntPtr cPtr, bool cMemoryOwn) : base() ", // Constructor used for wrapping pointers
            "{\n    swigCMemOwn = cMemoryOwn;\n",
            "    swigCPtr = cPtr;\n",
            "  }\n",
            NIL);
    ...
    if (tm) {
      ...
      if(isRootException)
      {
        Replaceall(destruct, "base.Dispose();", "");
      }
      if (*Char(destruct))
        Printv(proxy_class_def, "\n  public ", derived /*& !isRootException*/ ? "override" : "virtual", " void ", destruct_methodname, "() ", destruct, "\n", NIL);
    }
    Delete(attributes);
    Delete(destruct);
    ...
    if(derived){
      Printv(imclass_cppcasts_code,"\n  [DllImport(\"", (unmanagedDllName? unmanagedDllName: module_class_name), "\", EntryPoint=\"CSharp_", proxy_class_name ,"To", baseclass ,"\")]\n", NIL);
  ...
  virtual int classHandler(Node *n) {
    ...
    isRootException = !Strcmp(baseException, proxy_class_name);
...
C# Options (available with -csharp)\n\
     -namespace <name> - set name of namespace for proxy classes to <name>\n\

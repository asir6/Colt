...
local int unzlocal_getByte( DWFInputStream* pStream, int *pi)
{
    unsigned char c;
...
local unzFile unzLocal_OpenFile(const DWFString& zPath, unzIndex* locator)
{
    ...
    if (err!=UNZ_OK)
    {
        ...
        {
            _DWFCORE_THROW( DWFInvalidTypeException, L"Bad zip file" );
        }
...
*/
int ZEXPORT unzOpenCurrentFile (unzFile file, const DWFString& zPassword)
{

...
throw( DWFException )
{
    ...
    if (mp_params)
    {
        anNewParameters = new float[3*mp_pointcount];
    ...
    HU_OptimizeShell(  mp_pointcount,                       // number of shell vertices
                      ...
                      (mp_params == NULL) ? 0 : 3,          // parameter width
        ...
        if (anNewNormals || anNewParameters)
        {
            ...
            for (;iPoint < nOriginalPoints; iPoint++)
            {
                ...
                if (iVertex != -1)
                {
                    ...
                    if (anNewNormals)
                    {
                        anNewNormals[iVertex] = mp_normals[iPointVertex];               // i
                        anNewNormals[iVertex + 1] = mp_normals[iPointVertex + 1];       // j
                        anNewNormals[iVertex + 2] = mp_normals[iPointVertex + 2];       // k
                    }
                    ...
                    if (anNewParameters)
                    {
                        anNewParameters[iVertex] = mp_params[iPointVertex];             // u
                        anNewParameters[iVertex + 1] = mp_params[iPointVertex + 1];     // v
                        anNewParameters[iVertex + 2] = mp_params[iPointVertex + 2];     // w
                    }
                }
            }
...
TK_Status TK_Shell::read_advanced (BStreamFileToolkit & tk) alter {
    ...
    auto        eb_decompress_configs configs = {

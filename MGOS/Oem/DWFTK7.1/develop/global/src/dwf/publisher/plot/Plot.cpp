...
throw()
        : DWFPublishableSection(zPlotTitle, zPlotSource, zPlotSourceID)
...
throw( DWFException )
{
    if (pW2DStream == NULL)
    {
        _DWFCORE_THROW( DWFInvalidArgumentException, L"No stream provided" );
    }
...
_DWFTK_API
void
DWFPlot::getPlotExtents( vector<double>& rExtents )
const
throw()
{
    rExtents = _oExtents;
...
_DWFTK_API
void
DWFPlot::getPlotClip( vector<double>& rClip )
const
throw()
{
    rClip = _oClip;
...
throw( DWFException )
{
    if (_pW2DStream == NULL)
    {
        _DWFCORE_THROW( DWFNullPointerException, L"No stream bound to plot object" );
    }

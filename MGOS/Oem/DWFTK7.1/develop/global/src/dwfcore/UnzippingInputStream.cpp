...
throw()
{
    ...
    if (_bOwnDescriptor && _pDescriptor)
    {
...
throw( DWFException )
{
    ...
    if (unzLocateFile(_pUnzipStream, zArchivedFile, 1) == UNZ_OK)
    {
            ...
            // open the file
            //
        if (unzOpenCurrentFile(_pUnzipStream, zPassword) == UNZ_OK)
        {
        ...
        {
            _DWFCORE_THROW( DWFIOException, L"Failed to open archived file" );
        }
    }
    else
    {
        _DWFCORE_THROW( DWFIOException, L"File not found in archive" );
    }
...
throw( DWFException )
{
    if (_bFileOpen == false)
    {
        _DWFCORE_THROW( DWFIOException, L"No file open for unzipping" );
    }
...
throw( DWFException )
{
    ...
    if (nBytesRead < 0)
    {
        ...
        {
            _DWFCORE_THROW( DWFIOException, L"Internal zip library error occured" );
        }
    }

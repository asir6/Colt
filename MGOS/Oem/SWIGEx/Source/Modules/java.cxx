...
char cvsroot_java_cxx[] = "$Header: /cvsroot/SWIG/Source/Modules/java.cxx,v 1.49 2003/12/13 00:13:10 cheetah Exp $";
...
class JAVA : public Language {
  ...
  String *rootObject;
  String *jniDllName;  //Name of the C++ dll containing the JNI implementation
  ...
  void emitCatchSequence(File* f, ExceptionClass* e)
  {
      ...
      if(Len(package) > 0)
          qualifiedName = string(Char(package_path)) + "/" + e->name;
      else
          qualifiedName = e->name;

	  Printf(f, "\t{\n");
	  Printf(f, "\t\t(void)e; exName = \"%s\";\n", qualifiedName.c_str());
	  Printf(f, "\t}\n");
  }
	...
	void emitThrowFunction(File* f)
	{
		...
		Printf(f, "void ThrowJavaExceptionWrapper(JNIEnv *jenv, %s* e)\n{\n", baseException);
		...
		Printf(f, "\tjclass exCls = jenv->FindClass(exName);\n");
		Printf(f, "\tjmethodID ctorId = jenv->GetMethodID(exCls, \"<init>\", \"(JZ)V\");\n");
		Printf(f, "\tjthrowable exObj = (jthrowable)jenv->NewObject(exCls, ctorId, (jlong)e, (jboolean)1);\n");
		Printf(f, "\tjenv->Throw(exObj);\n");
      ...
      if (argv[i]) {
        if (strcmp(argv[i],"-package") == 0) {
          ...
          skip_throws_clause = true;
			...
			if (argv[i+1]) 
			{
				jniDllName = NewString("");
				Printf(jniDllName, argv[i+1]);
				Swig_mark_arg(i);
				Swig_mark_arg(i+1);
				i++;
			} 
          ...
          if (argv[i+1]) {
            rootObject = NewString("");
            Printf(rootObject, argv[i+1]);
            Swig_mark_arg(i);
            Swig_mark_arg(i+1);
            i++;
          } else {
  ...
  virtual int top(Node *n) {
	...
	// Generate the ObjectFactory class
	{
		String *filen = NewStringf("%s%s.java", SWIG_output_directory(), "ObjectFactory");
		File *f_factory = NewFile(filen,"w");
		if(!f_factory) {
			Printf(stderr,"Unable to open %s\n", filen);
			SWIG_exit(EXIT_FAILURE);
		}
        ...
        if(Len(package) > 0)
    		Printv(f_factory, "package ", package, ";\n\n", NIL);
        ...
        if(Len(package) > 0)
            packageSpec = NewStringf("%s.", package);

		for(map<string, int>::const_iterator it = clsIds.begin(); it != clsIds.end(); it++)
		{
            Printf(f_factory, "		classMap.put(new Integer(%d), Class.forName(\"%s%s\").getConstructor(new Class[] { Long.TYPE, Boolean.TYPE }));\n", it->second, (Len(package) > 0? packageSpec: ""), it->first.c_str());
		}
    ...
    // Generate the intermediary class
    {
      ...
      if (n_dmethods > 0) {
        ...
        Printf(f_im, "    System.loadLibrary(\"%s\");\n", jniDllName);
        Printf(f_im, "  }\n");
      }
	  else
	  {
  ...
  virtual int functionWrapper(Node *n) {
	...
	//first function wrapper, add an extra wrapper for getClassId() -- function already generated in the C++ code at that point
	if(firstFuncWrapper)
	{
		firstFuncWrapper = false;
		Printf(imclass_class_code, "  public final static native int getClassId(long jarg1);\n");
	}
    ...
    if(!native_function_flag)
	{
        ...
        if(proxy_class_name && disposeCode)
        {
            Replaceall(f->code, "delete arg1", disposeCode);
        }
	}
  ...
  void emitProxyClassDefAndCPPCasts(Node *n) {
	...
	bool isBaseException = baseException && !Strcmp(baseException, proxy_class_name);
    ...
    if(!isBaseException)
    {
        ...
        Printv(proxy_class_def,
            ...
            derived || *Char(pure_baseclass)/*(isRootObject || derived || *Char(pure_baseclass))*/?
            " extends " : 
            "",
            baseclass,
            pure_baseclass,
    ...
    char* pproxy_class_def = Char(proxy_class_def);
    char* pproxy_class_code = Char(proxy_class_code);
  ...
  void proxyClassFunctionHandler(Node *n) {
    ...
    if ((tm = Swig_typemap_lookup_new("javaout",n,"",0))) {
	  ...
	  char* pimclassname = Char(imclass_name);

...
/// </summary>
class MG_PLATFORMBASE_API MgPoint3D : public MgSerializable
{
    MG_DECL_DYNCREATE();
    DECLARE_CLASSNAME(MgPoint3D)
...
INTERNAL_API:
    ...
    MgPoint3D(const double X, const double Y, const double Z);

    /// <summary>
    /// Copy constructor
    /// </summary>
    /// <param name="vec">The vector to be copied.</param>
    MgPoint3D(const MgPoint3D& vec);
    ...
    bool isEqualTo(const MgPoint3D& vec, double tolerance = 1.0e-12) const;

    /// <summary>
    /// Returns the normal of the vector.
    /// </summary>
    /// <returns>A new normalized vector.</returns>
    MgPoint3D normal(const double& tolerance = 1.0e-12) const;
    ...
    MgPoint3D& normalize(const double& tolerance = 1.0e-12);
    ...
    double dotProduct(const MgPoint3D& vec) const;
    ...
    MgPoint3D crossProduct(const MgPoint3D& vec) const;
    ...
    MgPoint3D& operator=(const MgPoint3D& vec);
    ...
    /// <param name="vec">The vector to compare to.</param>
    /// <returns>True if the vectors are equal; otherwise false.</returns>
    bool operator==(const MgPoint3D& vec) const;
    ...
    MgPoint3D operator*(const double scale) const;
    ...
    /// <returns>The scaled vector.</returns>
    friend MgPoint3D operator*(const double scale, const MgPoint3D& vec)
    {
        return MgPoint3D(vec.x * scale, vec.y * scale, vec.z * scale);
    }
    ...
    MgPoint3D operator+(const MgPoint3D& vec) const;

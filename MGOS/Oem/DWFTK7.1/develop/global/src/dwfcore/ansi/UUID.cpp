...
DWFUUIDImpl_ANSI::DWFUUIDImpl_ANSI( const DWFUUIDImpl_ANSI& uuid )
throw()
                : _zUUID( uuid._zUUID )
{
    DWFCORE_COPY_MEMORY( _tUUID, uuid._tUUID, sizeof(_tUUID) );
}

DWFUUIDImpl_ANSI&
DWFUUIDImpl_ANSI::operator=( const DWFUUIDImpl_ANSI& uuid )
throw()
{
    _zUUID = uuid._zUUID;
...
throw( DWFException )
{
    ...
    if (pNew == NULL)
    {
        _DWFCORE_THROW( DWFMemoryException, L"Failed to allocate clone" );
    }

...
#define _DWFTK_PAPER_TEMP_BUFFER_BYTES  1024
    ...
    {
		//DNT_Start
        rSerializer.addAttribute( DWFXML::kzAttribute_Units, (_eUnits == eMillimeters ? L"mm" : L"in") );

        _DWFCORE_SWPRINTF( zTempBuffer, _DWFTK_PAPER_TEMP_BUFFER_BYTES, L"%.17g", _nWidth );
        rSerializer.addAttribute( DWFXML::kzAttribute_Width, zTempBuffer );

        _DWFCORE_SWPRINTF( zTempBuffer, _DWFTK_PAPER_TEMP_BUFFER_BYTES, L"%.17g", _nHeight );
        rSerializer.addAttribute( DWFXML::kzAttribute_Height, zTempBuffer );
        ...
        if ((_anClip[0] != 0.0) ||
            ...
            (_anClip[3] != 0.0))
        {
            _DWFCORE_SWPRINTF( zTempBuffer, _DWFTK_PAPER_TEMP_BUFFER_BYTES, L"%.17g %.17g %.17g %.17g", _anClip[0], _anClip[1], _anClip[2], _anClip[3] );

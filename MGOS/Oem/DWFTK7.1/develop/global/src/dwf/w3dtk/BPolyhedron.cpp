    ...
    while (in < end) {
        if (!exists || (*exists++ & flag)) {
            ...
            if (in[0] < -EPS || in[0] > EPS ||
                in[2] < -EPS || in[2] > EPS)
                out[0] = (float)atan2(in[2],in[0]); //theta
            else
                out[0] = 0;
            out[1] = (float)acos(in[1]);            //phi
        }
        else {
            out[0] = out[1] = 0;
        }
...
TK_Status TK_Polyhedron::mangle_point_attributes(BStreamFileToolkit & tk, int pointmap_count, int *pointmap) {
    ...
    if (HasVertexParameters()) {
        ...
        for (i = 0; i < pointmap_count; i++) {
            index = pointmap[i];
            temparray[ i*3 + 0 ] = mp_params[ index*3 + 0 ]; 
            temparray[ i*3 + 1 ] = mp_params[ index*3 + 1 ]; 
            temparray[ i*3 + 2 ] = mp_params[ index*3 + 2 ]; 
        }
        delete [] mp_params;

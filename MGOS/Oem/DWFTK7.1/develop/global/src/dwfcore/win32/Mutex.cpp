...
throw( DWFException )
{
    if (_bInit)
    {
        _DWFCORE_THROW( DWFUnexpectedException, L"Mutex already initialized" );
    }
...
throw( DWFException )
{
    if (_bInit == false)
    {
        _DWFCORE_THROW( DWFUnexpectedException, L"Mutex not initialized" );
    }
...
throw( DWFException )
{
    ...
    _DWFCORE_THROW( DWFNotImplementedException, L"This feature is not available on Win9x configurations" );

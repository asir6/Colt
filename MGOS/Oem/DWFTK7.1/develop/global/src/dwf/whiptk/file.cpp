...
WT_Result    WT_File::read_ascii(double & val)
{
    ...
    case Accumulating_Exponent:
        ...
        m_read_ascii_double_value *= pow(10.0,exponent);
        // We are done!!
        break;
    default:
        return WT_Result::Internal_Error;
...
{
#if WD_BIGENDIAN
    WD_CHECK (read(sizeof(WT_Logical_Point) * count, (WT_Byte *) points));

    WT_Logical_Point *  pos = points;
    for (int loop = 0; loop < count; loop++)
    {
    ...
    return WT_Result::Success;
#else
    return read(sizeof(WT_Logical_Point) * count, (WT_Byte *) points);
#endif
}
...
{
#if WD_BIGENDIAN
    WD_CHECK (read(sizeof(WT_Logical_Point_16) * count, (WT_Byte *) points));

    WT_Logical_Point_16 *  pos = points;
    for (int loop = 0; loop < count; loop++)
    {
        pos->m_x = WT_Endian::adjust(pos->m_x);
        pos->m_y = WT_Endian::adjust(pos->m_y);
    ...
    return WT_Result::Success;
#else
    return read(sizeof(WT_Logical_Point_16) * count, (WT_Byte *) points);
#endif
}

...
throw( DWFException )
{
    ...
    if (_fp != NULL)
    {
        _DWFCORE_THROW( DWFIOException, L"File already open" );
    }
    ...
    if (IsWindows9x())
    {
        ...
        {
            _DWFCORE_THROW( DWFMemoryException, L"Failed to allocate temporary string buffer" );
        }
        ...
        if (nMBCSBytes == 0)
        {
            _DWFCORE_THROW( DWFStringException, L"Failed to convert filename using local ANSI code page" );
        }
        ...
        if (_fp == NULL)
        {
            _DWFCORE_THROW( DWFIOException, L"Failed to open file" );
        }

        struct _stat tStat;
        if (_stat( apMBCSFilename, &tStat) == -1)
        {
            _DWFCORE_THROW( DWFIOException, L"Failed to stat file" );
        }
...
throw( DWFException )
{
    if (_fp == NULL)
    {
        _DWFCORE_THROW( DWFIOException, L"No file currently open" );
    }
    ...
    if (nBytesRead != nBytesToRead)
    {
        if (ferror(_fp) != 0)
        {
            _DWFCORE_THROW( DWFIOException, L"File read error" );
        }
    }
...
throw( DWFException )
{
    ...
    {
        _DWFCORE_THROW( DWFIOException, L"Failed to seek" );
    }
...
throw( DWFException )
{
    ...
    if (nBytesWritten != nBytesToWrite)
    {
        if (ferror(_fp) != 0)
        {
            _DWFCORE_THROW( DWFIOException, L"File write error" );
        }
    }
...
throw( DWFException )
{
    ...
    if (::fflush( _fp ) != 0)
    {
        _DWFCORE_THROW( DWFIOException, L"File flush error" );
    }
}

...
throw()
{
    return DWFCORE_ALLOC_OBJECT( DWFStringVectorIterator(_oPropertyRefs) );
}
...
throw( DWFException )
{
    if (zNode.bytes() == 0)
    {
        _DWFCORE_THROW( DWFInvalidArgumentException, L"Instance node must be specified" );
    }
    ...
    if (pInst == NULL)
    {
        _DWFCORE_THROW( DWFMemoryException, L"Failed to allocate instance" );
    }
...
throw()
{
    return DWFCORE_ALLOC_OBJECT( DWFStringVectorIterator(_oChildIDs) );
}
...
throw( DWFException )
{
    ...
    {
        //
        // use the fast high res timer to get an arbitrary number
        //
        uint64_t nTick = DWFTimer::Tick64();

        //
        // "make" a string with these 8 bytes
        //
        _zID.assign( (const char*)&nTick, 8 );
    }
}
